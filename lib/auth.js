...
module.exports = function auth (options) {
  ...
  function findById (args, done) {
    return auth.load$({id: args.id}, (err, auth) => {
      ...
      if (!auth) return done(null, {msg: 'not found'});
      return done(null, {auth: seneca.util.clean(auth)});
    });
  }
  ...
  function login (args, done) {
    return auth.load$({email: args.email}, (err, value) => {
      if (err) return done(err);
      if (!value) return done(null, {msg: 'Login or Password invalid'});
      if (!bcrypt.compareSync(args.password, value.password)) return done(null, {msg: 'Login or Password invalid'});
      return done(null, {auth: true});
    });
  }

  function setToken (args, done) {
    if (!args.token) return done(null, {msg: 'Token invalid'});
    if (!args.id) return done(null, {msg: 'ID invalid'});
    redis.set(args.token, args.id);
    redis.expire(args.token, (60 * 60) * 24);
    return done(null, {setToken: true});
  }
  ...
  function getToken (args, done) {
    ...
    redis.get(args.token, (err, value) => {
      if (err) return done(err);
      if (!value) return done(null, {msg: 'Token not found'});
  ...
  function create (args, done) {
    ...
    return auth.save$((err, resauthult) => {
      if (err) return done(err);
      return done(null, {auth: seneca.util.clean(auth)});
    });
  }
